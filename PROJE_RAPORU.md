...
### CENG302 YAZILIM MÜHENDİSLİĞİ DÖNEM SONU PROJE RAPORU

---

## Proje Başlığı
**Dağıtık Mimaride Tasarım Kalıpları ile KVKK Uyumlu Güvenli Log Yönetimi Ara Katman Yazılımı**

## Geliştirici
**Abdullah Toprak KILIÇ** Bilgisayar Mühendisliği Öğrencisi
...
Bu projede, finansal sistemlerin (özellikle yüksek işlem hacmine sahip borsaların) en kritik gereksinimlerinden biri olan güvenli, ölçeklenebilir ve rol tabanlı log analizi ve denetimi (audit) problemi ele alınmıştır.
...
# 2. KULLANILAN YAZILIM MİMARİ STRATEJİLERİ (ARCHITECTURAL PATTERNS)
...
## 2.1. Dağıtık İstemci-Sunucu Mimarisi (Distributed Client-Server Architecture)
...
## 2.2. Boru Hattı ve Filtre Mimarisi (Pipe and Filter Architecture)
...
## 3.1. Singleton Tasarım Kalıbı (Teklik Kalıbı)
...
Python dilinin `__new__` metodu ezilerek (override edilerek), uygulamanın çalışma zamanında (runtime) bellekte sadece tek bir nesne örneğinin (instance) barındırılması ve tek bir soket hattının yönetilmesi garanti altına alınmıştır.

---

## 3.2. Strategy Tasarım Kalıbı (Strateji Kalıbı)
...
Bu biçimlendirme lojistiğini tek bir `if-else` bloğuna yığmak SOLID'in **Open/Closed Principle (Gelişime açık, değişime kapalı)** prensibini doğrudan ihlal ederdi.
...
## 4.1. Yapay Zeka Ajanı (AI Agent) ile Ortak Geliştirme Metodolojisi
...
## 4.2. Performans Odaklı Erken Filtreleme (Early Dropping)

Sistem stres testi altındayken saniyede binlerce log üretmektedir.

Ağır metin işleme (Regex) ve disk I/O işlemleri sistem performansını düşüren en büyük darboğazlardır (bottleneck).
...
## 4.4. Önem Derecesine Göre Vurgulama (Highlighting)
...
- **Throughput (Saniye Başına İşlem Kapasitesi):** Yapılan testlerde tek çekirdek üzerinde çalışan Python ara katman sunucusunun, saniyede ortalama **800 - 950 adet logu** soketten okuyup, parse edip, elenenleri ayırıp, kalanları KVKK süzgecinden geçirerek 3 farklı dosyaya eş zamanlı yazabildiği ve kararlı bir şekilde çalıştığı gözlemlenmiştir.
